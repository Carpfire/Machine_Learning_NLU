...
training_args = TrainingArguments(
        output_dir="/scratch/lc3919/train_res",
        do_train = True,
        do_eval=True,
...
trainer = Trainer(model_init=finetuning_utils.model_init,args=training_args, train_dataset=train_data, eval_dataset=val_data, tokenizer=tokenizer, compute_metrics=finetuning_utils.compute_metrics)
...
search  = trainer.hyperparameter_search(direction = "maximize",search_alg =HyperOptSearch(),compute_objective = my_obj, hp_space=my_hp_space_ray, n_trials=5, mode="max", metric="eval_accuracy")
